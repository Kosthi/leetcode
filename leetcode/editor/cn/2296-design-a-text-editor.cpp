...
//
//  Related Topics 栈 设计 链表 字符串 双向链表 模拟 👍 69 👎 0


namespace solution2296 {
    ...
    public:
        ...
        void addText(string text) { left_.append(text); }

        int deleteText(int k) {
            auto len = min(k, (int) left_.size());
            left_.resize(left_.size() - len);
            return len;
        }
        ...
        string cursorLeft(int k) {
            ...
            return left_.substr(max(0, (int) left_.size() - 10));
        }

        string cursorRight(int k) {
            auto len = min(k, (int)right_.size());
            while (len--) {
                left_ += right_.back();
                right_.pop_back();
            }
